...
def rescore(run, extractor=extract, grader=None):
    ...
    for row in rows:
        ...
        if grader is None:
            ...
            assessment = {"extracted": str(predicted) if predicted is not None else None,
                          "diagnostic_reward": int(predicted is not None and predicted == target)}
        else:
            assessment = grader(row["response"], row["ground_truth"])
        groups.setdefault(row["prompt_id"], []).append(assessment["diagnostic_reward"])
        details.append({"id": row["id"], "original_reward": row["reward"], **assessment})
...
def main():
    ...
    parser.add_argument("--rule", choices=["numeric-box", "final-numeric-v1", "final-numeric-v2", "final-numeric-v3", "math-verify-v1"], default="numeric-box")
    args = parser.parse_args()
    extractor = extract
    grader = None
    verifier_metadata = None
    if args.rule == "math-verify-v1":
        import sys
    ...
    report = {"rule_version": args.rule, "rule": "Single numeric box" if args.rule == "numeric-box" else "Conservative explicit final numeric answer; matching boxes/statements permitted",
              ...
              "conditions": [rescore(run, extractor, grader) for run in runs]}
    if args.rule.startswith("final-numeric-"):
    ...
    if verifier_metadata is not None:
        report["verifier"] = verifier_metadata
        report["rule"] = "Pinned Math-Verify parse then verify, boxed-priority first match"
        report["purpose"] = "Candidate evaluation; existing format-independent correctness labels remain valid"
        report["extractor_sha256"] = hashlib.sha256(
